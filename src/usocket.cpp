...
void UClientSocket::setNagle(bool on)
{
    ...
    if (setsockopt(sockNum, IPPROTO_TCP, TCP_NODELAY, (void*) &nodelay,sizeof(nodelay)) == -1)
...
void UClientSocket::setReuse(bool yes)
{
    ...
    if (setsockopt(sockNum,SOL_SOCKET,SO_REUSEADDR,(char *)&op,sizeof(op)) == -1) 
...
{
    int op = size;
    if (setsockopt(sockNum,SOL_SOCKET,SO_SNDBUF,(char *)&op,sizeof(op)) == -1) 
        throw SockException("Unable to set SNDBUF");
}
